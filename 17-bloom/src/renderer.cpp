...
OpenGLRenderer::OpenGLRenderer(const int windowWidth, const int windowHeight) {
    ...
    window     = glfwCreateWindow(windowWidth, windowHeight, "17-bloom", nullptr, nullptr);
    if (!window) {
        const char *desc;
    ...
    mainShaders = std::make_unique<GLGraphicsShaders>(
        "../17-bloom/shaders/blinn-phong.vert",
        "../17-bloom/shaders/blinn-phong.frag"
    );
    basicColorShaders = std::make_unique<GLGraphicsShaders>(
        "../17-bloom/shaders/basic-color.vert",
        "../17-bloom/shaders/basic-color.frag"
    );
    hdrQuadShaders = std::make_unique<GLGraphicsShaders>(
        "../17-bloom/shaders/final-combine-hdr.vert",
        "../17-bloom/shaders/final-combine-hdr.frag"
    );
    skyboxShaders = std::make_unique<GLGraphicsShaders>(
        "../17-bloom/shaders/skybox.vert",
...
void OpenGLRenderer::render() {
...

    {
        ...
        basicColorShaders->setUniform("bloom_threshold", bloomThreshold);
    ...
    // we don't render the skybox this time, as it's easier to see bloom results without the skybox
...
void OpenGLRenderer::initHdrFramebuffer() {
    ...
    { // thresholded bloom color
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, hdrBloomTextureID, 0);
    }
