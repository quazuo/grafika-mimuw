...
void main() {
    ...
    vec2 ao_tex_coords = gl_FragCoord.xy / window_size;
    float ambient_occlusion = (use_ssao == 1) ? texture(ssao_texture, ao_tex_coords).r : 1.0;

    const float ambient_factor = 0.3f;

    vec2 directional_light_factors = calc_directional_light();

    vec3 ambient  = ambient_factor * ambient_occlusion * base_color;
